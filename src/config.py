...
class Config:
    ...
    # Subscribers tagged with this Buttondown tag get the Sunday digest instead
    # of daily sends (they chose "weekly" on the subscribe form).
    BUTTONDOWN_WEEKLY_TAG = os.getenv("BUTTONDOWN_WEEKLY_TAG", "weekly")

    # Editorial persona & plain-language style
    # Named human curator shown in footers ("drafted with AI, curated by X").
    # Reuters Institute DNR 2025: readers accept AI-assisted news far better
    # when a named human is visibly in the loop — never ship an anonymous
    # "generated by AI" footer.
    NEWSLETTER_EDITOR_NAME = os.getenv("NEWSLETTER_EDITOR_NAME", "")
    NEWSLETTER_TAGLINE = os.getenv("NEWSLETTER_TAGLINE", "The world's news from every side")
    ...
    # run compact with a computed coverage mini-bar. One-story issues starved
    # the SEO surface (1 indexable topic/day instead of 3) and read thin.
    NEWSLETTER_TARGET_STORIES = max(1, min(3, int(os.getenv("NEWSLETTER_TARGET_STORIES", "3"))))
